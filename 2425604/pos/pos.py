    ...
    def generate_tax_file(self):
        ...
        try:
            ...
            with open(tax_file, 'w', newline='') as csvfile:
                writer = csv.writer(csvfile)
                writer.writerow(['Bill Number','Item Code', 'Internal Price', 'Discount', 'Sale Price', 'Quantity', 'Line Total','Checksum'])
                        ...
                        with open(bill_file, 'r', newline='') as bill_csv:
                            ...
                            for row in reader:
                                ...
                                if in_items_section and row[0] != 'Grand Total':
                                    ...
                                    transaction = f"{bill_number},{item_code},{internal_price},{discount},{sale_price},{quantity},{line_total}"
                                    checksum = self.calculate_checksum(transaction)

                                    # Write transaction with checksum
                                    writer.writerow(
                                        [bill_number, item_code, internal_price,discount, sale_price, quantity, line_total, checksum])
